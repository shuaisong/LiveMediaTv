...
public class TCPointSeekBar extends RelativeLayout {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        //draw  bg
        bgRectF.left = mSeekBarLeft;
        bgRectF.right = mSeekBarRight;
        bgRectF.top = mBgTop;
        bgRectF.bottom = mBgBottom;
        canvas.drawRoundRect(bgRectF, mRoundSize, mRoundSize, mNormalPaint);

        //draw progress
        pRecf.left = mSeekBarLeft;
        pRecf.top = mBgTop - 2;
        pRecf.right = mThumbRight - mHalfDrawableWidth;
        pRecf.bottom = mBgBottom + 2;
