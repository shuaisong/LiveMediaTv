...
public class BookSearchActivity extends BaseActivity implements BookSearchContact.View, View.OnFocusChangeListener, RadioGroup.OnCheckedChangeListener {
    @Inject
    ...
    private BaseQuickAdapter<BookRecmendSeachBean, BaseViewHolder> hotAdapter;
    ...
    protected void initView() {
        ...
        tvSearch.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (!TextUtils.isEmpty(s)) {
                    ...
                    presenter.getBooks(tvSearch.getText().toString());
                }
            }
        });
        ...
        if (PreferenceManager.getInstance().isDefaultLanguage()) {
            String string = getString(R.string.book_search_hint);
    ...
    private void initResult() {
        GridLayoutManager gridLayoutManager = (GridLayoutManager) searchResult.getLayoutManager();
        if (gridLayoutManager != null) {
            gridLayoutManager.setSpanCount(5);
        }
        resultAdapter = new BaseQuickAdapter<BookSearchBean, BaseViewHolder>(R.layout.item_book_list) {
            @Override
        ...
        resultAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                BookSearchBean item = resultAdapter.getItem(position);
                if (item == null) return;
                Intent intent = new Intent(BookSearchActivity.this, PlayBookActivity.class);
                intent.putExtra("id", item.getB_id());
                startActivity(intent);
                presenter.addSearchNum(item.getB_id());
            }
        });
    ...
    private void initHotList() {
        hotAdapter = new BaseQuickAdapter<BookRecmendSeachBean, BaseViewHolder>(R.layout.item_search_list) {
            @Override
            protected void convert(BaseViewHolder helper, BookRecmendSeachBean item) {
                helper.setText(R.id.content, Util.showText(item.getB_title(), item.getB_title_z()));
        ...
        hotAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                BookRecmendSeachBean item = hotAdapter.getItem(position);
                if (item == null) return;
                Intent intent = new Intent(BookSearchActivity.this, PlayBookActivity.class);
                intent.putExtra("id", item.getB_id());
                startActivity(intent);
    ...
    @Override
    public void showBooks(List<BookSearchBean> bookSearchBeans) {
        lineRecommend.setVisibility(View.GONE);
        swipeRefreshLayout.setVisibility(View.VISIBLE);
        swipeRefreshLayout.setRefreshing(false);
        resultAdapter.setNewData(bookSearchBeans);
    }

    @Override
    public void showBookOrders(List<BookRecmendSeachBean> serach_book) {
        hotAdapter.setNewData(serach_book);
    }
